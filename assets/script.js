...
async function loadCatalogProducts(lang) {
  try {
    ...
    catalogProducts = (translations[lang].products || []).map((product) => ({
      key: product.key,
      name: product.name,
...
function buildPriceMarkup(product, lang) {
  const promoLabel =
    lang === "kk" ? "Акция" : lang === "en" ? "Sale" : "Акция";
  const hasDiscount = product.discountPercent > 0 || product.oldPrice;
  const oldPriceValue =
    product.oldPriceFormatted ||
    (product.oldPrice
      ? `${String(product.oldPrice).replace(/\B(?=(\d{3})+(?!\d))/g, " ")} ₸`
      : "");
    ...
    <div class="price-block">
      ...
      ${
        hasDiscount && oldPriceValue
          ? `<span class="price-old">${oldPriceValue}</span>`
          : ""
      }
      ${
      ...
      ${
        product.isPromotion
          ? `<span class="price-promo">${promoLabel}</span>`
          : ""
      }
    </div>
  `;
...
function renderCatalog(lang) {
  ...
  const products = catalogProducts.length
    ? catalogProducts
    : translations[lang].products.map((product) => ({
        key: product.key,
        name: product.name,
        description: product.description,
        price: product.price,
      ...
      (product) => `
        <article class="product-card reveal">
          <div class="product-media">
            <img src="${product.image || productImages[product.key]}" alt="${product.name}" />
          </div>
...
async function setLanguage(lang) {
  ...
  await loadCatalogProducts(lang);
  renderCatalog(lang);
  renderReviews(lang);
  renderFaq(lang);
